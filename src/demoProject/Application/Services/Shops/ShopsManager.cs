...
public class ShopsManager : IShopsService
{
    ...
    public async Task<Shop?> GetShopByUserIdAsync(
        int userId, 
        Func<IQueryable<Shop>, IIncludableQueryable<Shop, object>>? include = null,
        bool withDeleted = false,
        bool enableTracking = true,
        CancellationToken cancellationToken = default)
    {
        Shop? shop = await _shopRepository.GetAsync(s=>s.UserId==userId, include, withDeleted, enableTracking, cancellationToken);
        return shop;
    }
